...
def register(request):
    if request.method == 'POST':
        ...
        # Save the farmer and link the fingerprint
...
def login_view(request):
    if request.method == 'POST':
        if 'face_image' in request.POST:
            ...
            # Convert the image into an array and extract face encoding
            image_array = np.array(image)
            face_encoding = face_encodings(image_array)
